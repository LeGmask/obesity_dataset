...
class KMeans(BaseClassifier):
	...
	def __init_centroids_plusplus(self, X: np.ndarray | pd.DataFrame) -> None:
		...
		:param X: the data to initialize the centroids with
		"""
		self.centroids = [X[np.random.randint(len(X))]]  # randomly select first centroid

		for _ in range(self.k - 1):
			distances = np.array([np.min([np.sum((x - c) ** 2) for c in self.centroids]) for x in X])
	...
	def __assign_labels(self, X: np.ndarray | pd.DataFrame) -> None:
		...
		for point in range(len(X)):
			idx = np.argmin([KMeans.euclidean_distance(X[point, :], centroid) for centroid in self.centroids])
